...
//Global variables declared by TANVI
//Used book Computer Networking by Kurose
int state=0,state_b=0;
int  state_ack_a=1;
...
void A_input(struct pkt packet)
{
	...
	printf("\nmy_checksum is %d state is %d",my_checksum, state);
	//Check state and alter state accordingly
	if (state==0)
	{
		if ((my_checksum==packet.checksum) && ((packet.acknum==1)&&(packet.seqnum==0)))
		{
			state=1;
		...
		if ((my_checksum!=packet.checksum) || (packet.acknum==1))
		{
			//do nothing
		}
	}
	...
	{
		if ((my_checksum==packet.checksum) &&  ((packet.acknum==1) &&(packet.seqnum==1)))
		{
			state=0;
		...
		if ((my_checksum!=packet.checksum) || (packet.acknum==0))
		{
			//do nothing
		}
	}
...
void B_input(struct pkt packet)
{
	...
	//Act according to state, state_b correspond to two states in FSM
	if(state_b==0)
	{
		if((my_checksum==packet.checksum )&& (packet.seqnum ==0))
		{
			tolayer5(1,packet.payload);
			B_application++;
			new_packet.acknum=1;//1 tells packet is acknowledgement 
			new_packet.seqnum=0;
		...
		if((my_checksum!=packet.checksum )|| (packet.seqnum ==1))
		{
	...
	else if(state_b==1)
	{
		if((my_checksum==packet.checksum )&& (packet.seqnum ==1))
		{
			...
			for(int i=0;i<20;i++)
				new_packet.checksum+=new_packet.payload[i];
			tolayer3(1,new_packet);
			printf("\n new_packet details sent to layer3- seqnum %d acknum %d checksum  %d payload %s \n", new_packet.seqnum, new_packet.acknum, new_packet.checksum, new_packet.payload);
			state_b=0;
